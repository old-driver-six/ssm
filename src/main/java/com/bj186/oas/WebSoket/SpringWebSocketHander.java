 ...
 * 这样就可以用用户在系统中,注册的信息方便进行通信.
 */
public class SpringWebSocketHander extends TextWebSocketHandler {
    //存储WebSoket和用户ID的map
    private static final List<Massgs<String>> news=new ArrayList<>();   //存放离线消息的集合
    ...
    public void afterConnectionEstablished(WebSocketSession socketSession){
        ...
        /**
         * 这里写自己的业务代码,可以是将离线的消息推送给用户
         */
        for (Massgs massgs : news) {
            WebSocketSession session = users.get(massgs.getUid());
            if(session!=null){
                sendMessageToUser(massgs.getUid(),new TextMessage(massgs.getTableKey()+""));
                news.remove(massgs);        //消息发完了,应该将消息从集合中删除出去
            }
        }
    }
    ...
    public void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        if(!"".equals(massgs.getUid())){     //判断当前用户是否在线,如果不在线就将消息缓存在消息集合中
            ...
            if(massgs.getUid().equals("TOUSERS")){      //判断是否是群发消息,如果不是群发消息,就是单发消息
                sendMessageToUsers(new TextMessage(massgs.getTableKey()+""));
            }else {
                try {
                    sendMessageToUser(massgs.getUid(),new TextMessage(massgs.getTableKey()+""));
    ...
    /**
     * 给某个用户发送消息,通过用户传递过来的id和map中存放的id进行比对,
     * 找到要给用户发的消息.
     * 可以在这个类中维护一个ArrayList缓存用户离线的消息
